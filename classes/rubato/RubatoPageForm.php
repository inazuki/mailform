...
class RubatoPageForm extends RubatoPage {
 ...
 */
	...
	protected $_encode;					// 内部文字エンコーディング

	//------------------------------------------------------
	// クラス継承時の割込みアクション（オーバーライド用）
	//------------------------------------------------------
	public function actionInit()         {}  // 初期実行
	public function actionFirst()        {}  // 初回のみ
	public function actionParseAfter()   {}  // データ解析直後（初回以外）
	public function actionCommon()       {}  // 分岐アクション前（データ処理済み）
	public function actionInput()        {}  // 分岐アクション「入力」
	public function actionConfirm()      {}  // 分岐アクション「確認」
	public function actionComplete()     {}  // 分岐アクション「完了」
	public function actionCompleteOnce() {}  // 分岐アクション「完了」※１度だけ
	...
	public function __construct($useDB=true, $isSession=true) {
		...
		$this->_encode = mb_internal_encoding();
	}
		...
		foreach ($this->_config as $id => $conf) {
			...
			if (array_key_exists("conf", $conf)) {
				...
				foreach ($conf['conf'] as $childId => $childConf) {
					...
					if (!is_null($this->getRequest($childId))) $this->_data[$childId] = $this->getRequest($childId);
				...
				// 単一項目の場合
				//------------------------------------------
				if (!is_null($this->getRequest($id))) $this->_data[$id] = $this->getRequest($id);
	...
	**/
	...
	public function action() {
		$this->actionInit(); //※割込みアクション：初期実行

		if ($this->_action == "default") {
			// セッションのデータを削除
			$_SESSION[$this->_sessionName] = array();

			$this->actionFirst(); //※割込みアクション：初回のみ
			...
			$this->actionParseAfter(); //※割込みアクション：データ解析直後（初回は実行されない）
			if (count($this->_error)) $this->_action = "input";

			// データのアサイン（入力時のみ未結合）
			if ($this->_action == "input") $this->view->assign("data", $this->getData(null, false));
			else $this->view->assign("data", $this->getData(null, true));
		}

		$this->actionCommon(); //※割込みアクション：分岐アクション前（データ処理済み）

		switch ($this->_action) {
			case "input":
				if (count($this->_data)) $this->setSession('data', $this->_data);
				$this->actionInput(); //※割込みアクション：分岐アクション「入力」
				break;
			case "confirm":
				if (count($this->_data)) $this->setSession('data', $this->_data);
				$this->actionConfirm(); //※割込みアクション：分岐アクション「確認」
				break;
			case "complete":
				if (!$this->getSession('completed')) {
					$this->setSession('completed', true);
					$this->actionCompleteOnce(); //※割込みアクション：分岐アクション「完了」※1度だけ
				}
				$this->actionComplete(); //※割込みアクション：分岐アクション「完了」
				break;
		}

		$this->view->assign("list", $this->_list);
		$this->view->assign("error", $this->_error);
	}
