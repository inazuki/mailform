...
require_once 'RubatoPageForm.php';
require_once 'RubatoMail.php';


class RubatoPageMailForm extends RubatoPageForm {
	 ...
	 * 割込み処理：アクション「完了」※1回のみ
	 */
	public function actionCompleteOnce() {
		// 送信日時をセット
		$this->_config['SEND_DATE'] = array("name" => "送信日時");
		$this->_data['SEND_DATE'] = date("Y-m-d H:i:s");
		$this->view->assign("data", $this->getData(null, true));
	...
	public function actionMailSend() {
		...
		if (isset($this->_mailUser['to']) && isset($this->_mailUser['body'])) {
			...
			$mailObj->setBody($this->view->fetch($this->_mailUser['body']));
		...
		if (isset($this->_mailAdmin['to']) && isset($this->_mailAdmin['body'])) {
			...
			$mailObj->setBody($this->view->fetch($this->_mailAdmin['body']));
